...
{
    StackType<VertexType> VStack;
    QueType<VertexType> VQue;
    bool found = false;
...
{
    int fromIndex,toIndex,outNumber=0;
...
bool GraphType<VertexType>::FoundEdge(VertexType U, VertexType V)
{
    ...
    bool found=false;
    fromVertex=IndexIs(vertices,U);
    toVertex=IndexIs(vertices,V);
    if(edges[fromVertex][toVertex]!=NULL_EDGE || edges[toVertex][fromVertex]!=NULL_EDGE)
        found=true;
    return found;
}

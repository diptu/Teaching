...
template<class T>
void addGreaterValue(TreeNode<T> *root, int *sumNext)
{   // base case
    if(root == NULL){
        return;
    }
    // for right side of tree
    addGreaterValue(root->right, sumNext);
    *sumNext = *sumNext + root->info;
    root->info = sumNext;
    // for left side of tree
    addGreaterValue(root->left, sumNext);
}
template<class T>
void sumOfGreaterValue(TreeNode<T> *root)
{
    int sum = 0;

...
template <class ItemType>
void TreeType<ItemType>::SearchandInsert(TreeNode<ItemType>* originalRoot,TreeNode<ItemType>*& tempRoot,ItemType sum,ItemType item)
{
    if(tempRoot!=NULL)
    {
        if(tempRoot->info==item)
        {
            tempRoot->info=sum;
            root=originalRoot;
            return;
        }
        SearchandInsert(originalRoot,tempRoot->left,sum,item);
        SearchandInsert(originalRoot,tempRoot->right,sum,item);

    }
}

template <class ItemType>
void TreeType<ItemType>::InsertGREATER(ItemType item,int n)
{
    int sum=item;
    int temp;
    for(int i=0;i<n;i++)
    ...
    SearchandInsert(root,root,sum,item);
}
